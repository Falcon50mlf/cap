...
import { ArrowRight, Check, X, Box, Tag, MapPin, Megaphone } from 'lucide-react';
...
const ROUNDS: Round[] = [
  ...
  {
    brand: 'Decathlon',
    brief: 'Decathlon fête ses 50 ans. Place les 12 décisions du mix dans la bonne case.',
    cards: [
  ...
  {
    brand: 'Lush',
    brief: 'Lush, cosmétiques éthiques. Place les 12 décisions du mix dans la bonne case.',
    cards: [
      {
        id: 'l1',
        text: 'Ingrédients naturels, lutte contre les emballages plastique',
        expected: 'produit',
      },
    ...
    >
      <div className="flex items-center gap-2 mb-3 px-1">
        ...
        <div className="min-w-0">
          <div className="font-display font-bold text-base leading-none">{zone.label}</div>
...
function CardPill({
  ...
  return (
    ...
    >
      {validated && correct === true && <Check className="w-4 h-4 shrink-0 mt-0.5" />}
      {validated && correct === false && <X className="w-4 h-4 shrink-0 mt-0.5" />}
      <span>{text}</span>
    </div>
  );
}
