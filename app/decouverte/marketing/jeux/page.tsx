...
import { ArrowLeft, ArrowRight, Lock, PlayCircle, Users } from 'lucide-react';
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          >
            ...
            <p className="mt-6 text-snow/60 text-lg max-w-xl">
              Chaque mini-jeu simule une vraie situation pro. Joue à ton rythme, à la fin tu verras
              tes points forts.
            </p>
          </motion.div>
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-6">
              ...
              <div className="flex-1">
                ...
                <p className="text-snow/70">
                  Léa (Master Marketing, Mondelēz) et Tom (Doctolib, ex-Vinted) sont dispo pour 1-2
                  échanges par mois.
                </p>
              </div>
              <Link
...
function GameCard({ g, index }: { g: GameMeta; index: number }) {
  ...
  const inner = (
    ...
    >
      ...
      <h3 className="font-display font-bold text-xl tracking-tight mb-2 leading-tight">{g.name}</h3>
      <p className="text-snow/60 text-sm leading-relaxed mb-5 flex-1">{g.concept}</p>
  ...
  return live ? (
    ...
    <Link href={`/decouverte/marketing/jeux/${g.id}`} className="pointer-events-auto">
      {inner}
    </Link>
  );
}
