...
import { getSubmodule, type ContentSection } from '@/lib/schools-database';
...
export default function SubmodulePage({ params }: { params: { moduleId: string } }) {
      ...
      <section className="px-6 md:px-10 pt-8 pb-12">
        <div className="max-w-[820px] mx-auto">
          ...
          >
            ...
            <p className="text-snow/75 text-lg md:text-xl leading-relaxed">{sub.intro}</p>
...
function Section({ section }: { section: ContentSection }) {
  if (section.type === 'definition') {
    return (
      <div
        className="rounded-3xl p-6 md:p-7 border-l-4 bg-night-soft"
      ...
      >
        ...
        <p className="text-snow/85 text-base md:text-lg leading-relaxed">{section.body}</p>
      </div>
    );
  }
  ...
  if (section.type === 'callout') {
    ...
    const color = c.kind === 'warn' ? 'var(--coral)' : c.kind === 'tip' ? 'var(--sun)' : ACCENT;
    return (
      <div
      ...
      <div>
        ...
        <div className="space-y-3">
          {section.items?.map((item, i) => (
            <div key={i} className="rounded-2xl p-4 border border-night-200 bg-night-soft/40">
              <div className="font-display font-bold text-base mb-1" style={{ color: ACCENT }}>
                {item.label}
              </div>
              {item.value && <div className="text-snow/75 text-sm leading-snug">{item.value}</div>}
            </div>
          ))}
        </div>
      </div>
  ...
  if (section.type === 'intro') {
    return <p className="text-snow/85 text-lg md:text-xl leading-relaxed">{section.body}</p>;
  }
